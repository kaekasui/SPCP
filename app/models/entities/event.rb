...
class Event < ActiveRecord::Base
  ...
  validates_presence_of :name, :message => :missing_event_name
  ...
  # Attach given attachment to the event if it hasn't been attached already.
  #----------------------------------------------------------------------------
  def attach!(attachment)
    unless self.send("#{attachment.class.name.downcase}_ids").include?(attachment.id)
      if attachment.is_a?(Task)
        ...
        attachment.update_attribute(:event, self)
  ...
  def discard!(attachment)
    if attachment.is_a?(Task)
      ...
      attachment.update_attribute(:event, nil)
    end
  end
  ...
  # Make sure at least one user has been selected if the event is being shared.
  #----------------------------------------------------------------------------
  def users_for_shared_access
    errors.add(:access, :share_event) if self[:access] == "Shared" && !self.permissions.any?
  end
